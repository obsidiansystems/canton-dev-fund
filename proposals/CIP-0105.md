...
**Created:** 2026-04-20  
**Updated:** 2026-05-22  
**Implements:** CIP-0105 and optionally CIP-0116

---

## Abstract

CIP-0105 was approved on March 2, 2026. Phase 1 (off-chain transitional enforcement is active). Phase 2 (on-chain enforcement) remains unimplemented. Since initial submission of this proposal, CIP-0116 (Featured App Locking) has also been passed but is also unimplemented.
...
This proposal funds Obsidian Systems to design, build, deploy, and maintain the Phase 2 on-chain enforcement layer as a Splice contribution, satisfying the Working Group's functional requirements. Obsidian will lead development and own delivery outcomes; Working Group members who have active contributions will receive part of the allocation of this grant.
...
CIP-0116, optionally covered by this proposal, introduces objective on-chain criteria for Featured App designation, replacing manual Foundation governance with on-chain verifiable commitment. This resolves the same manual verification problems as SV locking Phase 1 but in the Featured App locking context.
...
In total, this proposal covers the full end-to-end locking lifecycle: from lock creation and delegated locking, through continuous weight evaluation and enforcement, to vesting, unlock, and withdrawal. This proposal includes the atomic Lock Substitution primitive needed for lender swaps, the authorization model for custodial and multi-party arrangements, the observability surface for compliance dashboards, and the Phase 1 to Phase 2 migration path. DA's Phase 2 capabilities and all finalized WG Section A requirements are in scope.

### Architectural Direction

The M1 design doc will translate the above functional requirements into a technical design approved by the Splice maintainer team. The following architectural commitments inform that design:
...
| Forthcoming Locking CIP                                                                                                              | Obsidian to implement this CIP as passed/updated and fall back to WG document if CIP is not passed within M1 timeframe             |
...
**Timeline:** Months 1–2
**Focus:** Produce a technical design doc and complete implementation  plan that satisfies the WG functional requirements within Splice architecture and wins maintainer buy-in.
...
### M2 — Contract Suite + Governance Foundations
...
- Governance automation stubs: scan APIs, trigger interfaces ready for M3
...
- Governance automation stubs reviewed by DA Splice team, which gates M3 entry

**Estimated amount:** 1,500,000 CC

### M3 — Governance Automations + Observability + Compliance

**Timeline:** Months 5–6
**Focus:** Automation, observability, and compliance surfaces making the M2 contract suite usable in operations.
...
- If CIP-0116 extension is approved: FA-specific threshold evaluation, breach lifecycle (immediate removal), FA beneficiary registration, lock-gated voting workflow, existing FA migration
...
**Estimated amount:** 1,500,000 CC (+ 1,000,000 CC if CIP-0116 extension approved)
...
### Optional CIP-0116 Extension

CIP-0116 (Featured App Locking) was approved May 20, 2026. The base proposal's locking primitive is parameterized to support FA locking; this extension delivers FA locking as a complete, deployed feature. If the committee elects to include CIP-0116 in scope, an additional 1,000,000 CC is added to M3 for FA-specific work. The timeline does not change.

**CIP-0116 requirements:**
...
| Line item                                              | Amount           |
| ------------------------------------------------------ | ---------------- |
| M1 — Technical design + Splice sign-off                | 1,500,000 CC     |
| M2 — Contract suite + governance foundations           | 1,500,000 CC     |
| M3 — Automations + observability + compliance          | 1,500,000 CC     |
| M4 — Mainnet deployment                                | 500,000 CC       |
| M5 — Post-Mainnet ownership (6 months)                 | 1,000,000 CC     |
| **Base total (CIP-0105)**                              | **6,000,000 CC** |
| Early completion bonus (M3 accepted by end of month 5) | 1,000,000 CC     |
| **Total with bonus**                                   | **7,000,000 CC** |
...
- **Digital Asset:** 300,000 CC — scoped for Splice maintainer design review, SV node debugging access during M3/M4, and SV coordination support
...
If CIP-0116 is included in scope:

| Line item                                              | Amount           |
| ------------------------------------------------------ | ---------------- |
| M1 — Technical design + Splice sign-off                | 1,500,000 CC     |
| M2 — Contract suite + governance foundations           | 1,500,000 CC     |
| M3 — Automations + observability + compliance          | 2,500,000 CC     |
| M4 — Mainnet deployment                                | 500,000 CC       |
| M5 — Post-Mainnet ownership (6 months)                 | 1,000,000 CC     |
| **Base total (CIP-0105 + CIP-0116)**                   | **7,000,000 CC** |
| Early completion bonus (M3 accepted by end of month 5) | 1,000,000 CC     |
| **Total with bonus**                                   | **8,000,000 CC** |

**Payment schedule:** Paid on milestone acceptance. M5 invoiced in two tranches (months 10 and 13) to align with ongoing support delivery.
...
**Early completion bonus:** 1,000,000 CC if M3 is accepted by end of month 5 (one month ahead of schedule). This triggers on engineering completion, not Mainnet deployment, because M4 depends on a governance vote and SV coordination outside Obsidian's control. Follows precedent of PR #107 (Traffic-Based App Rewards).
...
- **In-build (Months 1–7).** Bug fixes and design refinements absorbed within milestone scope. Test coverage and SCU compatibility maintained as build-time obligations.  
- **Post-Mainnet ownership (Months 8–13, M5).** Obsidian retains direct operational responsibility (bug fixes, SV feedback, incident response) closing with the recommended follow-on scope deliverable.  
- **Long-run (Month 14 onward).** Three candidate paths, confirmed at the close of M5 based on observed maintenance load and DA / Foundation direction:  
...
The long-run path is recommended in the M5 end-of-period report based on operational data from the first six months of Mainnet.
...
| Phase            | Composition                                         |
| ---------------- | --------------------------------------------------- |
| M1 (Months 1–2)  | Senior Daml engineer + architect                    |
| M2 (Months 3–4)  | Senior Daml engineer + mid-level Daml engineer      |
| M3 (Months 5–6)  | Senior Daml engineer + 1–2 mid-level Daml engineers |
| M4 (Month 7)     | Senior Daml engineer + mid-level Daml engineer      |
| M5 (Months 8–13) | Support ownership capacity (Daml engineer(s))       |
...
*Submitted April 29, 2026. Updated and resubmitted May 22, 2026. Prepared by Obsidian Systems for Canton Foundation Development Fund Grants Committee review.*  
